...
if ( ! class_exists( 'Kirki_Controls_Typekit_Control' ) ) {

    /**
     * Typekit control.
     */
    class Kirki_Controls_Typekit_Control extends Kirki_Customize_Control {
        ...
        public function enqueue() {
            wp_enqueue_script( 'kirki-typekit' );
        }
            ...
            <div class="wrapper">
                ...
                <# if ( data.default['font-family'] ) { #>
                    ...
                    <div class="font-family">
                        ...
                        <select {{{ data.inputAttrs }}} id="kirki-typekit-font-family-{{{ data.id }}}" placeholder="{{ data.l10n['select-font-family'] }}"></select>
                    </div>
                    <# if ( true === data.show_variants || false !== data.default.variant ) { #>
                        <div class="variant hide-on-standard-fonts kirki-variant-wrapper">
                            <h5>{{ data.l10n['variant'] }}</h5>
                            <select {{{ data.inputAttrs }}} class="variant" id="kirki-typekit-variant-{{{ data.id }}}"></select>
                        </div>
                    <# } #>
                    <# if ( true === data.show_subsets ) { #>
                        <div class="subsets hide-on-standard-fonts kirki-subsets-wrapper">
                            <h5>{{ data.l10n['subsets'] }}</h5>
                            <select {{{ data.inputAttrs }}} class="subset" id="kirki-typekit-subsets-{{{ data.id }}}"></select>
                        </div>
                    <# } #>
                <# } #>
                ...
                <# if ( data.default['text-align'] ) { #>
                    <div class="text-align">
                        ...
                        <input {{{ data.inputAttrs }}} type="radio" value="inherit" name="_customize-typekit-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
                            <label for="{{ data.id }}-text-align-inherit">
                                <span class="dashicons dashicons-editor-removeformatting"></span>
                                <span class="screen-reader-text">{{ data.l10n['inherit'] }}</span>
                            </label>
                        </input>
                        <input {{{ data.inputAttrs }}} type="radio" value="left" name="_customize-typekit-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
                            <label for="{{ data.id }}-text-align-left">
                                <span class="dashicons dashicons-editor-alignleft"></span>
                                <span class="screen-reader-text">{{ data.l10n['left'] }}</span>
                            </label>
                        </input>
                        <input {{{ data.inputAttrs }}} type="radio" value="center" name="_customize-typekit-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
                            <label for="{{ data.id }}-text-align-center">
                                <span class="dashicons dashicons-editor-aligncenter"></span>
                                <span class="screen-reader-text">{{ data.l10n['center'] }}</span>
                            </label>
                        </input>
                        <input {{{ data.inputAttrs }}} type="radio" value="right" name="_customize-typekit-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
                            <label for="{{ data.id }}-text-align-right">
                                <span class="dashicons dashicons-editor-alignright"></span>
                                <span class="screen-reader-text">{{ data.l10n['right'] }}</span>
                            </label>
                        </input>
                        <input {{{ data.inputAttrs }}} type="radio" value="justify" name="_customize-typekit-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
                ...
                <# if ( data.default['text-transform'] ) { #>
                    <div class="text-transform">
                        ...
                        <select {{{ data.inputAttrs }}} id="kirki-typekit-text-transform-{{{ data.id }}}">
